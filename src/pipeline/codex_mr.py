"""Run the MR reviewer step through the Codex CLI in headless mode."""
import base64
import logging
import os
import shutil
...
def run_codex_mr_review(
    ...
    if api_key:
        ...
        tmp_codex_home = tempfile.mkdtemp(
            dir=os.path.expanduser("~"), prefix=".codex-mr-review-"
        )
    ...
    os.close(tmp_fd)
    try:
        logger.info("Invoking Codex MR reviewer: model=%s", s.reviewer_model)
        ...
        try:
            ...
            raise CodexError(
                f"Codex MR reviewer timed out after {s.iteration_timeout_seconds}s"
            ) from exc

        logger.info(
            "Codex MR reviewer returned exit=%s in %.1fs",
            proc.returncode,
        ...
        try:
            ...
            raise CodexError(f"Could not read Codex MR review output: {exc}") from exc
